...
PTR int_fdopen(IRIX_FILE *f, int fd, const char *pathname, const char *mode)
{
    ...
    char *p = strchr(mode, '+');
    int flags;
    if      (p != NULL) flags = O_RDWR;
    else if (w != NULL) flags = O_WRONLY;
    else if (a != NULL) flags = O_WRONLY;
    else                flags = O_RDONLY;
    if (fd < 0)
    {
        if (w != NULL) flags |= O_CREAT | O_TRUNC;
        if (a != NULL) flags |= O_CREAT | O_APPEND;
    ...
    {
        case O_RDONLY: f->_flag = 0001; break;
        case O_WRONLY: f->_flag = 0002; break;
        case O_RDWR:   f->_flag = 0200; break;
    }
    return __ptr(f);
}
